	...
	public static void main(String args[]) {
		...
		java.awt.EventQueue.invokeLater(new Runnable() {
			public void run() {
				...
				try {
					client.connect("149.125.31.85", (short) 9999);
...
class ChatListener implements Runnable {
	...
	//	client.read2();
	//	ImageIcon img = new ImageIcon("E:/YES.jpg");  
	//	chatBox.insertIcon(img);
	...
	public void run() {
		...
		while (true) {
			...
			if ((line = client.read()) != null) {
				...
				}else if(line.startsWith("Please")){
					//String applyName = line.split(":")[1].split(",")[0];
					//line = line;
				client.readIMG();
				//ChatFrame chatFrame = new ChatFrame(client);
			//	chatFrame.setLocationRelativeTo(null);
				//String authName = client.getUsername();
			AuthorizeFrame authorizeFrame = new AuthorizeFrame(client,line);
			System.out.println("authorizeFrame外部接受的那行话是:"+line);
				authorizeFrame.setLocationRelativeTo(null);
				authorizeFrame.setVisible(true);
				System.out.println("authorizeFrame 显示完毕");
				} else {
