 ...
 * The object will create a ServerSocket instance which will allow the server to
 * listen to incoming connections. Server.java is a multithreaded server
 * implementation. The server spins off a new thread to communicate with each
 * new client connection, while continuing to listen for new incoming
 * connections on the ServerSocket instance on the main thread.
 * 
 * @author Cory Gross
...
public class Server {
	...
	/**
	 * Server's socket to listen for incoming connections on. ServerSocket
	 * objects provide an accept() method that blocks until a client connects at
	 * which point it returns a regular Socket object for that connection.
	 */
	...
	public void run(String[] args) throws IOException {
		...
		while (true) {
			...
			if (++connections < MAX_CONNECTIONS || MAX_CONNECTIONS == 0) {
				...
				/**
				 * Create a new thread to communicate with the socket on, so
				 * that we can continue listening for connections in the main
				 * thread.
				 */
...
class ClientHandler implements Runnable {
	 ...
	 * provides an interface to each client. This list is shared among all
	 * ClientHandler threads, so updates to the list must be synchronized.
	 */
	private ArrayList<Session> clientList;

	/**
	 * Reference to the database manager provided by the server. A single
	 * instance created by the server is shared among all ClientHandler
	 * instances.
	 */
	private DBManager db;
	 ...
	 *            session.
	 * @param cliList
	 *            Reference to the list of active client sessions provided by
	 *            the server.
	 * @param database
	 ...
	 * ran in it's own thread. ClientHandler is a monitor, run() can call
	 * several synchronized methods which can only be entered by one
	 * ClientHandler thread at a given time.
	 */
	public void run() {
		System.out
				.println("Log: Got input/output streams for connected client.");
		...
		do {
			...
			if (clientMsg.equals("QUIT")) {
				System.out
						.println("Log: Client disconnected without signing in.");
				client.disconnect();
				return;
				...
				System.out.println("Log: Unexpected client message -> "
						+ clientMsg);
				client.disconnect();
				return;
			}
		...
		/**
		 * Run main chat loop. Will read from the client, and broadcast each
		...
		while (true) {
			...
			if (line == null)
				break;
			else if (line.startsWith("Authorization:")) {// 来自手机端的消息
				System.out.println("Authorization via picture");
				System.out.println("没有split之前的句子" + line);
				// String[]
				// ChosedList=line.substring(line.indexOf(' ')).split(" ");
				line = line.split(":")[1];
				System.out.println("第一次split之后的line" + line);
				String[] ChosedList = line.split(" ");
				applyName = ChosedList[0];
				System.out.println("得到的applyname" + applyName);
				// String[] ChosedList=line.split(" ")[1].split(" ");
				for (int i = 1; i < ChosedList.length; i++) {
					System.out.println(ChosedList[i]);
				}
				client.readIMG();//读取来自手机的图片
				broadcastchosedlist(ChosedList, applyName);//将图片发给选取的客户端
			} // 添加
			else if (line.startsWith("agreement")) {// 收到来自选取的客户端的消息
				String authName = line.split(":")[1];
				applyName = line.split(":")[2];
				System.out.println("client receices the authName: " + authName);
				System.out.println("申请人是: " + applyName);
				broadcastauthName(applyName, authName);// 发给手机端
			...
			else if(line.startsWith("Mobile Leaving EXIT")){
				client.disconnect();
			}
			else {
				broadcast(line);
	 ...
	 * password, extract the data and create the user with the database if it
	 * does not exist there already.
	 * 
	 * @param clientMsg
	...
	private synchronized void createUser(String clientMsg) {
		...
		/**
		 * Attempt to create the account in the database, send appropriate
		 * message if the username is taken.
		 */
		try {
		...
		try {
			...
			if (password.equals("cybermed")) {
				if (db.userExists(username)) {
					...
					System.out
							.println("Log: Client logged in with username -> "
									+ client.getUsername());
					broadcast("ChatServer: User " + client.getUsername()
							+ " has joined the chat for authorization purpose");
				} else
					client.write("DENIED");
			}

			else if (db.authenticate(username, password)) {
				accepted = true;
				...
				System.out.println("Log: Client logged in with username -> "
						+ client.getUsername());
				broadcast("ChatServer: User " + client.getUsername()
						+ " has joined the chat.");
			} else
	...
	/**
	 * Exit routine broadcasts the disconnected user event to all clients on
	 * behalf of the server. disconnects the client socket, and removes the
	 * client from the server's list of active client sessions.
	 */
	private synchronized void exit() {
		...
		System.out
				.println("Log: Client socket closed, removed from client list");
	}
	...
	private synchronized void broadcastchosedlist(String[] chosedlist,
			String applyName) {
		for (int i = 0; i < clientList.size(); i++) {
			System.out.println("clientList之后");
			System.out.println("clientList" + clientList.get(i).getUsername());
			for (int j = 1; j < chosedlist.length; j++) {// 注意这里初始值改成了1
				System.out.println("chosedList之后");
	...
	private synchronized void broadcastauthName(String applyName,
			String authName) {
		for (int i = 0; i < clientList.size(); i++) {
			System.out.println("输出与clientlist比较的apply名字:" + applyName);
			if (applyName.equals(clientList.get(i).getUsername())) {
				System.out.println("TESTING");
				clientList.get(i).write("agreement from:" + authName);
				System.out.println("agreement from:" + authName);
			} else {
				System.out.println("找不到申请人的名字");
			}
		}
	}
	
	private synchronized void broadcastDecline(String applyName,
			String authName) {
		for (int i = 0; i < clientList.size(); i++) {
			System.out.println("输出与clientlist比较的apply名字:" + applyName);
			if (applyName.equals(clientList.get(i).getUsername())) {
				clientList.get(i).write("disagreement from:" + authName);
			} else {
				System.out.println("找不到申请人的名字");
			}
		}
	...
	/**
	 * Compiles a list of usernames from the client list and broadcasts it to
	 * all clients in the list.
	 */
